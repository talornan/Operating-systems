...
//use in - https://stackoverflow.com/questions/21405204/multithread-server-client-implementation-in-c
//and use in - https://dzone.com/articles/parallel-tcpip-socket-server-with-multi-threading
void *handle_connection(void *arg){
    ...
    do {
        ...
        if (-1 == recv(new_fd, (void*)command, sizeof(command), 0)) {
            perror("ERROR: recv");
            goto Exit;
        }
        ...
        if (send(new_fd, top_result, sizeof(top_result), 0) == -1){
            perror("ERROR: send");
            goto Exit;
        }
    } while(1);
...
int main(void)
{
    ...
    while(1) {  // main accept() loop
        ...
        if (pthread_create(&th, NULL, handle_connection, &new_fd) != 0) // Create a thread for each request.
        {
